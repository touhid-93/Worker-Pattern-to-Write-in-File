...
public class WorkerThree {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            FileWriter fileWriterLog = new FileWriter("workerThree_LOG.txt");
            ...
            if (isTrue == false) {
                ...
                System.out.println("Worker-3 is busy!");
            }

            bufferedWriter.write("worker - 3 - " + globalText + " - " + getDateTime());
            ...
            bufferedWriter.newLine();
            bufferedWriter.write("worker - 3 - " + INTERNAL_TEXT_LIST.get(callCount) + " - " + getDateTime());
            bufferedWriter.newLine();
            ...
            if (isTrue == false) {
                ...
                System.out.println("Worker-3 is free!");
            }
    ...
    private static boolean updateStateBusy() {
        try {
            ...
            if (connection != null) {
                ...
                int resultset = sqlCommand.executeUpdate("UPDATE worker SET state = 'busy' WHERE worker_name = 'worker-3'");
    ...
    private static boolean updateStateFree() {
        try {
            ...
            if (connection != null) {
                Statement sqlCommand = (Statement) connection.createStatement();
                int resultset = sqlCommand.executeUpdate("UPDATE worker SET state = 'free' WHERE worker_name = 'worker-3'");
    ...
    private static ArrayList<String> GetInternalTextList() {
        ...
        try {
            ...
            if (connection != null) {
                Statement sqlCommand = (Statement) connection.createStatement();
                ResultSet resultset = sqlCommand.executeQuery("SELECT text1, text2, text3, text4 FROM worker WHERE worker_name = 'worker-3'");
    ...
    private static String GetGlobalFilePath() {
        try {
            ...
            if (connection != null) {
                Statement sqlCommand = (Statement) connection.createStatement();
                ResultSet resultset = sqlCommand.executeQuery("SELECT file_path FROM worker WHERE worker_name = 'worker-3'");
        ...
        System.err.println("Error : OutOfScope Error (GetGlobalFilePath())");
        return null;
        ...
        try {
            ...
            if (index < internalTextList_size) {
                for (int i = index; i < internalTextList_size; i++) {
                    bufferedWriter.write("worker - 3 - " + internalTextList.get(i) + " - " + getDateTime());
                    bufferedWriter.newLine();
                }
                bufferedWriter.write("worker - 3 - Task Finished");
                bufferedWriter.newLine();
            } else {
                bufferedWriter.write("worker - 3 - Task Finished");
                bufferedWriter.newLine();
