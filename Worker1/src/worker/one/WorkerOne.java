...
public class WorkerOne {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            FileWriter fileWriterLog = new FileWriter("workerOne_LOG.txt");
            ...
            if (isTrue == false) {
                ...
                System.out.println("Worker-1 is busy!");
            }

            bufferedWriter.write("worker - 1 - " + globalText + " - " + getDateTime());
            ...
            bufferedWriter.newLine();
            bufferedWriter.write("worker - 1 - " + INTERNAL_TEXT_LIST.get(callCount) + " - " + getDateTime());
            bufferedWriter.newLine();
            ...
            if (isTrue == false) {
                ...
                System.out.println("Worker-1 is free!");
            }
    ...
    private static boolean updateStateBusy() {
        try {
            ...
            if (connection != null) {
                ...
                int resultset = sqlCommand.executeUpdate("UPDATE worker SET state = 'busy' WHERE worker_name = 'worker-1'");
    ...
    private static boolean updateStateFree() {
        try {
            ...
            if (connection != null) {
                Statement sqlCommand = (Statement) connection.createStatement();
                int resultset = sqlCommand.executeUpdate("UPDATE worker SET state = 'free' WHERE worker_name = 'worker-1'");
    ...
    private static ArrayList<String> GetInternalTextList() {
        ...
        try {
            ...
            if (connection != null) {
                Statement sqlCommand = (Statement) connection.createStatement();
                ResultSet resultset = sqlCommand.executeQuery("SELECT text1, text2, text3, text4 FROM worker WHERE worker_name = 'worker-1'");
    ...
    private static String GetGlobalFilePath() {
        try {
            ...
            if (connection != null) {
                Statement sqlCommand = (Statement) connection.createStatement();
                ResultSet resultset = sqlCommand.executeQuery("SELECT file_path FROM worker WHERE worker_name = 'worker-1'");
        ...
        try {
            ...
            if (index < internalTextList_size) {
                for (int i = index; i < internalTextList_size; i++) {
                    bufferedWriter.write("worker - 1 - " + internalTextList.get(i) + " - " + getDateTime());
                    bufferedWriter.newLine();
                }
                bufferedWriter.write("worker - 1 - Task Finished");
                bufferedWriter.newLine();
            } else {
                bufferedWriter.write("worker - 1 - Task Finished");
                bufferedWriter.newLine();
